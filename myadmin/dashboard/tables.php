...
$Cancelationtable='';
$i=0;
$quey_app=mysqli_query($conn,"SELECT * FROM cancellation  ORDER  BY id DESC ");
...
while($row=mysqli_fetch_assoc($quey_app)){
    ...
    $id=$row['id'];
    $reasons=$row['reason'];
    $specialInstruction=$row['specail'];
    $fkey=$row['f_key'];
    $quey=mysqli_query($conn,"SELECT * FROM not_approved WHERE id='$fkey' ");
    $row=mysqli_fetch_assoc($quey);
    $email=$row['email'];
    $project_id=$row['project_id'];

    $Cancelationtable.="<tr><td>$i</td><td>$domains</td><td>$reasons</td><td>$specialInstruction</td><td>$email</td>
        <td>$project_id</td><td>
        <button class='btn2' onclick='RemoveCancelation($id)' style='color: red'>X</button></td></tr>";
}
