...
if(isset($_SESSION['uname']) && $_SESSION['id']) {
    ...
    if ($row = mysqli_fetch_assoc($query)) {
        ...
        $file=$row['file_name'];
        $src='';
        if($file=="undefined"){$src="";}else{$src="<br><img style='margin-left: 10px; display:block;' id='images1' width='350px' src='http://shawod.co.za/img/$file'>";}
        if($img == 1){
        ...
        while($row2=mysqli_fetch_assoc($query2)){
            if($x > 0 && $x < $total-1) {
                ...
                $src2='';
                if($file2=="undefined"){$src2="";}else{$src2="<br><img style='margin-left: 10px; display:block;' id='images2' width='350px' src='http://shawod.co.za/img/$file2'>";}
        ...
        $src3='';
        if($file3=="undefined"){$src3="";}else{$src3="<br><img style='margin-left: 10px; display:none;' id='images3' width='350px' src='http://shawod.co.za/img/$file3'>";}
        ...
        if(mysqli_num_rows($query3) == 1){
            $msg =" <div><p class=\"bill\"><u>Billing Department</u></p><p class=\"sub\" style='font-size: 20px'>$sub</p><br><p style=\"margin-left:25px; font-size:18px;\">
                ...
                <input type=\"text\" id=\"text_input1\" class='text_input'><br><input type=\"file\" id=\"file\" class='file' onchange=\"add_file('file','filename1')\"><div class=\"photo\">Attach a snapshot</div>
                ...
                <span id=\"close1\" class='close'   onClick=\"closed('view1', 'prev1', 'reply1','pre_ms1')\" onmouseup=\"img_cls('images3')\">Collapse</span>
...
if(isset($_POST['billing'])){
     $id=$_POST['billing'];
    mysqli_query($conn,"UPDATE billing set o_key=0 WHERE f_key='$id'");
    $ii1=0;$ii2=0;$ii3=0;$ii4=0;
    $myquery=mysqli_query($conn,"SELECT `o_key` FROM billing WHERE f_key=' $myid'");
    while ($row1=mysqli_fetch_assoc($myquery)){
        $ii1+=$row1['o_key'];
    }
    $myquery1=mysqli_query($conn,"SELECT `o_key` FROM other WHERE f_key=' $myid'");
    while ($row2=mysqli_fetch_assoc($myquery1)){
        $ii2+=$row2['o_key'];
    }
    $myquery2=mysqli_query($conn,"SELECT `o_key` FROM hosting WHERE f_key=' $myid'");
    while ($row3=mysqli_fetch_assoc($myquery2)){
        $ii3+=$row3['o_key'];
    }
    $myquer3=mysqli_query($conn,"SELECT `o_key` FROM developer WHERE f_key=' $myid'");
...
if(isset($_POST['developer'])){
    $id=$_POST['developer'];
    mysqli_query($conn,"UPDATE developer set o_key=0 WHERE f_key='$id'");
    $ii1=0;$ii2=0;$ii3=0;$ii4=0;
    $myquery=mysqli_query($conn,"SELECT `o_key` FROM billing WHERE f_key=' $myid'");
    while ($row1=mysqli_fetch_assoc($myquery)){
        $ii1+=$row1['o_key'];
...
if(isset($_POST['hosting'])){
    $id=$_POST['hosting'];
    mysqli_query($conn,"UPDATE hosting set o_key=0 WHERE f_key='$id'");
    $ii1=0;$ii2=0;$ii3=0;$ii4=0;
...
if(isset($_POST['other'])){
    $id=$_POST['other'];
    mysqli_query($conn,"UPDATE other set o_key=0 WHERE f_key='$id'");
    $ii1=0;$ii2=0;$ii3=0;$ii4=0;
    $myquery=mysqli_query($conn,"SELECT `o_key` FROM billing WHERE f_key='$ids'");
    while ($row1=mysqli_fetch_assoc($myquery)){
        $ii1+=$row1['o_key'];
...
if(isset($_POST['text'])){
   ...
   if($dep=="billing") {
       mysqli_query($conn, "INSERT INTO `billing` VALUES (null,'','$t',0,'$filex','$c_id',1,0,'$dt')");
   }else if($dep=="developer"){
       mysqli_query($conn, "INSERT INTO `developer` VALUES (null,'','$t',0,'$filex','$c_id',1,0,'$dt')");
   }else if($dep=="hosting"){
       mysqli_query($conn, "INSERT INTO `hosting` VALUES (null,'','$t',0,'$filex','$c_id',1,0,'$dt')");
   }else if($dep=="other"){
       mysqli_query($conn, "INSERT INTO `other` VALUES (null,'','$t',0,'$filex','$c_id',1,0,'$dt')");
   }
}