...
<?php require 'email.php'; $u_id=$_GET['id']; ?>

<body>
<script>
    function _(x) {return document.getElementById(x)}
    function proced_pay(){
        var reference = _("reference").value,exp_date=_("exp_date").value,details=_("details").value,
            stage=_("stage").value,descrip=_("descrip").value,price=_("price").value,id="<?php echo $u_id; ?>";
        ...
        ajax.send("reference=" + reference + "&exp_date=" + exp_date + "&details=" + details + "&stage=" + stage +"&descrip="+ descrip +"&price="+ price +"&id=" + id );
...
<ul>
  <li id="bi" style="background-color: rgb(51,204,255)" onclick="bill('bill','dev','host','other')" ><span style="font-size: 30px" onclick="bill('bill','dev','host','other')">Billing <?php echo $bi ?></span></li>
    ...
    <li id="py" onclick="payment('bill','dev','host','other')"><span style="font-size: 30px" onclick="payment('bill','dev','host','other')">Payments</span></li>
</ul>
</div>
...
<div class="content">
   ...
   <div class="label" align="center">Welcome to <?php echo  $name ?> account</div>
    ...
    $web=$row[1];$host=$row[2];$dmn=$row[3];$ssl=$row[4];$order_num=0;
    $row_pay=mysqli_query($conn,"SELECT * FROM web_payment WHERE f_key='$user_id'");
    if(mysqli_num_rows($row_pay) >= 1){
        $order_num=mysqli_num_rows($row_pay);
    }else{
        $order_num=0;
    }
    ?>
    ...
    <div id="project" style="width:100%; height:auto;display:none; ">
        ...
        <div id="preview3" style="margin-left: -80px">
            ...
            <div id="frame1">
                <iframe src="http://shawod.co.za/client_project/<?php  $ids=$_GET['id'];
    ...
    <div id="pay" style="width:100%; height:auto;display:none">

        <div style="width:1300px;margin-left:-260px; margin-top: -30px">
            <div class="content">
                <h1 id="top" align="center" style="margin-left: 120px;" >Payments</h1>

                <div class="label" style="margin-left: 80px">
                    <span id="nowdue" style="color: silver" onClick="nwd()">Now due</span>&nbsp;&nbsp;
                    |&nbsp;&nbsp;<span id="prd" onClick="products()">Products</span>
                    &nbsp;|&nbsp;&nbsp;<span id="invoice" onClick="invoices()">Invoices</span>
                    |&nbsp;&nbsp;<span id="settings" onClick="sttng('<?php echo $_SESSION['id'] ?>')">Settings</span>
                </div>
                ...
                <div id="py_content">
                    <div id="tb">
                        ...
                        <table id="table" border=""  style="margin-top: -15px; margin-left: 90px">
                            ...
                            <tr>
                                <td><input type="text" style="width: 100px" id="reference"></td><td><?php echo $order_num+1; ?></td><td><input type="text" id="exp_date" style="width: 100px"></td><td>
                                    <textarea id="details"></textarea></td><td><input type="text" id="stage" ></td><td><textarea id="descrip"></textarea></td><td>R<input type="text" id="price" style="width: 50px"></td><td>
                                   <button onclick="proced_pay()">Procced payment</button>
                                </td>
                            </tr>
                        </table>
