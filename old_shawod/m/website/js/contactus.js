...
function texts(vl,rx) {
    if(_(vl).innerHTML.trim() != ""){
        if(_(vl).innerHTML.trim().length <= 30){
            _(rx).innerHTML="Your message must write 30 or more characters";
...
function send_data(nam,email,contact,text) {
        ...
        if (_(text).innerHTML.trim() == "") {_(text).style.border = "1px solid red"}else{_(text).style.backgroundColor = "white"}
        ...
        if (_(nam).value.trim() != "" && _(email).value.trim() != "" && _(text).innerHTML.trim() != "") {
            ajax_post(nam, email, contact, text);
...
function ajax_post(n,e,c,t) {
    _("cont_btn").style.display="none";
    _("reference").style.display="block"
    _("reference").innerHTML="Please waite..."
    ...
    ajax2.send("name="+_(n).value+"&emails="+_(e).value+"&contact="+_(c).value+"&text="+_(t).innerHTML);
}